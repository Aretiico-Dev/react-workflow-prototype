interface AdminTwoFactorResetRejectedConfirmationProps {
  request: {
    aretiicoId: string;
    legalName: string;
    email: string;
...
export function AdminTwoFactorResetRejectedConfirmation({ request, rejection }: AdminTwoFactorResetRejectedConfirmationProps) {
  const rejectedAtLabel = new Date(rejection.rejectedAt).toLocaleString('en-GB', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
    hour: '2-digit',
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded border border-[#e0e0e0] shadow-[1px_0_20px_rgb(0_0_0_/_8%)] p-8 mt-8">
      <div className="mb-8">
        <h2 className="text-[1.75rem] font-semibold text-[#212121] mb-2">2FA Reset Rejected</h2>
        <p className="text-[0.875rem] text-[#616161]">
          This request was rejected and the full review context is preserved for future reference.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
          ...
          <div className="rounded border border-[#e0e0e0] bg-[#fafafa] p-6">
            ...
            {request.linkedUser ? (
              ...
              <div className="rounded border border-dashed border-[#e0e0e0] bg-white p-4">
                <p className="text-[0.875rem] text-[#616161]">No account had been linked at the time of rejection.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded border border-red-200 bg-red-50 p-6">
            <h3 className="text-[1rem] font-semibold text-[#212121] mb-4">Rejection details</h3>
            <div className="space-y-3">
              <div className="rounded border border-red-200 bg-white p-3">
                <p className="text-[0.75rem] text-[#616161] mb-1">Rejected by</p>
                <p className="text-[0.875rem] font-medium text-[#212121]">{rejection.rejectedBy}</p>
              </div>
              <div className="rounded border border-red-200 bg-white p-3">
                <p className="text-[0.75rem] text-[#616161] mb-1">Rejected at</p>
                <p className="text-[0.875rem] font-medium text-[#212121]">{rejectedAtLabel}</p>
              </div>
              <div className="rounded border border-red-200 bg-white p-3">
                <p className="text-[0.75rem] text-[#616161] mb-1">Reason</p>
                <p className="text-[0.875rem] text-[#212121] whitespace-pre-wrap">{rejection.reason}</p>
              </div>
            </div>
          </div>
          ...
          {request.reviewerDecisionContext && (
            <div className="rounded border border-[#e0e0e0] bg-[#fafafa] p-6">
              <h3 className="text-[1rem] font-semibold text-[#212121] mb-4">Reviewer notes</h3>
              {request.reviewerDecisionContext.flag && (
                <div className="mb-3 rounded border-l-4 border-[#ffc107] bg-[#fff8e1] p-3">
                  <p className="text-[0.75rem] font-medium text-[#212121] mb-1">Flag raised for decision maker</p>
