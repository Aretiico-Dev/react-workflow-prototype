...
const progressLabels = { family: "Family", subject: "Subject", group: "Use Case", product: "Product", crypto: "Crypto" };
...
export default function AviationProductWizardSpec() {
  ...
  const configurationFlows = ["manual", "ready", "acme", "scep", "mdm"];
  const currentStepId = flow === "select" ? currentKey : flow === "enrollment" ? "enrollment" : configurationFlows.includes(flow) ? "configuration" : "subject-details";

  function goToProductStep(index) {
    const next = {};
    const auto = {};
    steps.slice(0, index).forEach((key) => {
      if (selection[key]) next[key] = selection[key];
      if (autoSelected[key]) auto[key] = true;
    });
    setSelection(next);
  ...
  const aviationSteps = [
    ...steps.map((key, index) => ({
      ...
      onClick: selection[key] ? () => goToProductStep(index) : undefined,
    })),
    {
      id: "subject-details",
      label: "Subject Details",
      onClick: stepIndex >= steps.length ? () => setFlow("subject") : undefined,
    },
    {
      id: "enrollment",
      label: "Enrolment",
      onClick: flow === "enrollment" || enrollment || configurationFlows.includes(flow) ? () => setFlow("enrollment") : undefined,
    },
    {
      id: "configuration",
      label: "Configuration",
